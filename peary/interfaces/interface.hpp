...
public: 
  ...
  //Write data to a device containing internal registers
  //If readout is intergralpart of write operations, the read values a returned by function. 
  std::vector<DATA_T> write(const ADDRESS_T& address, const REG_T& reg, const std::vector< DATA_T>& data) = 0;
  ...
  std::vector<DATA_T> read(const ADDRESS_T& address, const unsigned int& length = 1) = 0;

  //Read number of data words form a register of the given device
  std::vector<DATA_T> read(const ADDRESS_T& address, const REG_T reg, const unsigned int& length = 1) = 0;
