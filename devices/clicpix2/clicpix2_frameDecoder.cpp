...
void clicpix2_frameDecoder::extractColumns(std::vector<clicpix2_frameDecoder::WORD_TYPE>::const_iterator& data, std::vector<clicpix2_frameDecoder::WORD_TYPE>::const_iterator dataEnd){
  std::array< std::array<pixelReadout, 8>, CLICPIX2_ROW * 2> pixels_dc; //stores results of the processed doube columns (up to 8 if compression is enabled)
  std::array<int,8> row_index = {0, 0, 0, 0, 0, 0, 0, 0};  //8 independent (in case of compression) counters navigating through the <pixels_dc>
  std::array<int,8> row_slice = { CLICPIX2_PIXEL_SIZE-1, CLICPIX2_PIXEL_SIZE-1, CLICPIX2_PIXEL_SIZE-1, CLICPIX2_PIXEL_SIZE-1,
				  CLICPIX2_PIXEL_SIZE-1, CLICPIX2_PIXEL_SIZE-1, CLICPIX2_PIXEL_SIZE-1, CLICPIX2_PIXEL_SIZE-1,};  //8 independent
...
#define DC_COUNTER_INIT  ( 2 * ( CLICPIX2_ROW * CLICPIX2_PIXEL_SIZE + CLICPIX2_ROW / CLICPIX2_SUPERPIXEL_SIZE ) + 1)
  std::array<int,8> dc_counter = { DC_COUNTER_INIT, DC_COUNTER_INIT, DC_COUNTER_INIT, DC_COUNTER_INIT,
				   DC_COUNTER_INIT, DC_COUNTER_INIT, DC_COUNTER_INIT, DC_COUNTER_INIT}; //8 independent (in case of compression) 
  ...
  std::array<int,8> sp_counter = {0, 0, 0, 0, 0, 0, 0, 0}; //number of pixels in the processed super-pixel for the given column
...
void clicpix2_frameDecoder::processDCbit(std::array< std::array<pixelReadout, 8>, CLICPIX2_ROW * 2> &pixels_dc, int & dc_counter,
