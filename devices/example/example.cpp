...
void example::powerOn() {
  ...
  //_hal->write(exampleDict.getAddress("vthreshold"),_config.Get("sample-registers",std::vector<uint32_t>{EXAMPLE_DAC_VEC}));
}
...
void example::exampleCall() {
  ...
  // Vectors can be read directly from the config and passed to an interface
  //_hal->write(0,_config.Get("sample-registers",std::vector<uint32_t>{EXAMPLE_DAC_VEC}));
